...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	// Frame package that is checked to verify hit
	...
	{
		// too far back for server side rewind
	...
	{
		// search until Older < HitTime < Younger
	...
	if (Older->GetValue().Time == HitTime) // not likely
	{
                                                              ...
                                                              ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	if (World)
	{
		...
		else // no headshot, check other hitboxes
		{
			for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
			{
				if (HitBoxPair.Value != nullptr)
	...
	const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	else // BodyShot, check other boxes
	{
		for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
		{
			if (HitBoxPair.Value != nullptr)
                                                                            ...
                                                                            const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	// enable collision for all boxes and disable head box
	for (auto& Frame : FramePackages)
	{
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	if (Character && Character->GetEquippedWeapon() && HitCharacter && Confirm.bHitConfirm)
	{
		UGameplayStatics::ApplyDamage(
			HitCharacter,
	...
	const TArray<FVector_NetQuantize>& HitLocations, float HitTime, AWeapon* DamageCauser)
{
	...
	for (auto& HitCharacter : HitCharacters)
	{
		...
		{
			float BodyShotDamage = Confirm.HeadShots[HitCharacter] * DamageCauser->GetDamage();
